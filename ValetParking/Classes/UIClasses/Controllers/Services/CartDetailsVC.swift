...
class CartDetailsVC: UIViewController {
    ...
    @objc func LocationPickAction(){
        let vc = UIStoryboard(name: "Services", bundle: nil).instantiateViewController(withIdentifier: "LocationPickUpVC") as! LocationPickUpVC
...
class CartListTVC: UITableViewCell{
    ...
    @IBOutlet weak var RemoveCartImg: UIImageView!
...
extension CartDetailsVC{
    func CartDetailsAPI(){
        ...
        if Connectivity.isConnectedToInternet
        {
            ...
            Alamofire.request(APIEndPoints.CartDetails, method: .post, parameters: params, encoding: JSONEncoding.default, headers: nil).responseJSON { apiResponse in
                            ...
                            if let result = results{
                                ...
                                if count != "0"{
                                    ...
                                    for list in cartList{
                                        ...
                                        let model = CartDetailsModel(id: id, providerId: providerId, customerId: customerId, serviceId: serviceId, serviceName: serviceName, serviceDescription: serviceDescription, serviceCost: serviceCost, requiredTime: requiredTime, serviceStatus: serviceStatus, serviceType: serviceType, pickupAvailable: pickupAvailable, serviceImages: serviceImages)
    ...
    func PlaceOrderAPI(){
        ...
        {
            SKActivityIndicator.show("Loading...")
            Alamofire.request(APIEndPoints.PlaceOrder, method: .post,parameters: params,encoding: JSONEncoding.default,headers: nil).responseJSON { apiResponse in
                print("PlaceOrderAPIResponse --- \(apiResponse)")
                switch apiResponse.result{
                case .success(_):
                    SKActivityIndicator.dismiss()
                    if let apiDict = apiResponse.value as? [String:Any]{
                        print("PlaceOrderDic---\(apiDict)")
                        let status = apiDict["status"] as? String ?? ""
                        let message = apiDict["message"] as? String ?? ""
                        if status == "success" {
                            if message == "nomobilenumber"{
                                let vc = UIStoryboard(name: "Mechbrain", bundle: nil).instantiateViewController(withIdentifier: "AddMobileNoVC") as! AddMobileNoVC
                                vc.modalPresentationStyle = .overCurrentContext
                                self.present(vc, animated: false, completion: nil)
                            } else{
                                let odID = apiDict["order_id"] as? String ?? ""
                                self.view.makeToast(message)
                                let vc = UIStoryboard(name: "Services", bundle: nil).instantiateViewController(withIdentifier: "OrderSuccessVC") as! OrderSuccessVC
                                vc.OrderID = odID
