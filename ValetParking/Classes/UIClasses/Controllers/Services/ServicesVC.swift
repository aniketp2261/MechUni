...
///Services sheet delegate
protocol ServicesVCDelegate {
    /// function to navigate to searchServices view controller
...
protocol ServicesCatVCDelegate{
    func ServicesNavigationBack()
}
struct CategoriesModel {
    let categoryName,categoryImage,categoryType,categoryTypeName,id,v : String
    let categoryActive : Bool
}
struct NearbyServicesModel{
    let id, serviceCost, serviceProviderId, serviceName, serviceDescription, serviceType, serviceCategory, serviceCateoryId: String
    let pickupavailable: Bool
}

class ServicesVC: UIViewController, UIAlertViewDelegate {
    ...
    var ServicesSheetVCDelegate: ServicesVCDelegate? = nil
    ...
    override func viewDidLoad() {
        ...
        ServicesImg.image = UIImage(named: "Services")
        ...
        OrdersCollectionView.register(UINib(nibName: "MyOrdersCVC", bundle: nil), forCellWithReuseIdentifier: "MyOrdersCVC")
        ...
        MechbrainImg.isUserInteractionEnabled = true
        ...
        LocationBtn.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(NearbyLocationAction)))
    ...
    @objc func NearbyLocationAction(){
        ServicesRad = false
        LocationRad = true
    ...
    @objc func HomeAction(){
        print("HomeAction")
        ServicesSheetVCDelegate?.ServicesHomeClickAction()
    }
    @objc func LocationAction(){
        print("LocationAction")
        for nearby in nearbyPlacesArray{
            ServicesSheetVCDelegate?.ServicesSearchPlaceAction(nearbyModel: nearby)
            break
        }
    }
    @objc func ParkingAction(){
        print("ParkingAction")
        ServicesSheetVCDelegate?.ServicesParkingClickAction()
    }
    @objc func MechbrainAction(){
        print("MechbrainAction")
        ServicesSheetVCDelegate?.ServicesMechBrainAction()
    }
    func SkipLoginPopUp(){
    ...
    func getNearbyServices(lat:String,lng:String,workingDay:String){
        ...
        if Connectivity.isConnectedToInternet
        {
            Alamofire.request(APIEndPoints.getNearbyServiceProviders, method: .post, parameters: params, encoding: JSONEncoding.default,headers: nil).responseJSON { apiResponse in
    ...
    func getServicesCategories(){
        if Connectivity.isConnectedToInternet
        {
            ...
            let params:[String: Any] = ["category_for":"service"]
            Alamofire.request(APIEndPoints.getServicesCategories, method: .get, parameters: params, headers: nil).responseJSON { apiResponse in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if LocationRad == true{
            if serviceArray.count != 0{
                ServicesSheetVCDelegate?.ServicesNearbyServicesClick(provider: serviceArray[indexPath.row])
            }
        } else{
            if categoryArray.count != 0{
                ServicesSheetVCDelegate?.ServicesServicesByCategory(Category: categoryArray[indexPath.row])
            }
        }
    }
}
...
extension ServicesVC: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ServicesSheetVCDelegate?.ServicesSearchListAction()
